...
def train_all_models(X_enc, y_class, y_reg):
    ...
    X_train_r, X_test_r, y_train_r, y_test_r = train_test_split(
        X_enc, y_reg, test_size=0.2, random_state=SEED, shuffle=False
    )
...
##############################################################################
# 8. Funkcja do wczytywania i prezentowania metryk
##############################################################################
def show_metrics():
    """
    Wczytuje i wyświetla metryki z plików CSV (classification_results.csv i regression_results.csv),
    o ile one istnieją.
    """
    import pandas as pd
    from tabulate import tabulate
    ...
    if os.path.isfile("classification_results.csv"):
        ...
        print("\nBrak pliku classification_results.csv")
    ...
    if os.path.isfile("regression_results.csv"):
        ...
        print("\nBrak pliku regression_results.csv")

##############################################################################
# 9. Ładowanie wszystkich modeli (klasyfikacja + regresja)
##############################################################################
